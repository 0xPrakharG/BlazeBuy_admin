...
function SettingsPage({ swal }) {
  ...
  const [featuredProductId, setfeatruedProductId] = useState("");
  ...
  async function fetchAll() {
    ...
    await axios.get("/api/settings?name=featuredProductId").then((res) => {
      setfeatruedProductId(res.data.value);
    });
    ...
    <Layout>
      ...
      {!isLoading && (
        <>
          ...
          <select
            value={featuredProductId}
            onChange={(ev) => setfeatruedProductId(ev.target.value)}
          >
            {products.length > 0 &&
          ...
          <label>Shipping Fee (in INR)</label>
          <input
            type="number"
            value={shippingFee}
